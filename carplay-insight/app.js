...
function renderSolutionEvidence(report) {
  ...
  const statusNames = { product_backed: "有公开商品方案", forum_fallback: "论坛链接回退", empty: "暂无可核验方案" };
  const groups = (solution.groups || []).map((group) => {
    ...
    const emptyMarkup = !products && !forumLinks ? "<div class='panel evidence-empty'>该形态和痛点目前没有可展示的商品或论坛链接。</div>" : "";
    return "<article class='solution-group'><header class='solution-group-head'><div><span class='eyebrow'>"
      ...
      + escapeHTML(group.note || "") + "</p>" + productMarkup + forumMarkup + emptyMarkup
...
function renderReport(report) {
  ...
  root.innerHTML = [renderExecutive(report), renderProductShape(report), renderSolutionEvidence(report), renderEvidenceDrivenUsers(report), renderEvidenceDrivenOpportunities(report), renderDefinition(report), renderEvidence(report)].join("");
  renderIcons(root);
  bindReportActions();
