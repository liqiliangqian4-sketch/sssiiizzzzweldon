...
function renderShapeCards(clusters) {
  ...
  if (!rail) return;
  const developmentById = {
    oval: "优先中尺寸，避免遮挡主镜",
    "d-semi-oval": "对比纯椭圆与现有 XL 的视野",
    square: "小批对比圆角矩形的点击表现",
    rhombus: "小批测试独立点击与转化",
    round: "只做胶材、成本与包装优化",
    heart: "只评估颜色与礼赠变体",
    ...
    return `
      <article class="shape-card ${cluster.verdictTone}">
        ...
        <div class="shape-card-body">
          ...
          <div class="shape-compact-fact action"><span>开发建议</span><strong>${developmentById[cluster.id] || cluster.action}</strong></div>
...
function enableBudgetEmbed() {
  ...
  const reportHeight = () => {
    ...
    frame = requestAnimationFrame(() => {
      ...
      window.parent.postMessage({ type: "embedded-module-size", key: "blind-routes", height }, "*");
    });
  };

  new ResizeObserver(reportHeight).observe(section);
  document.querySelectorAll("#routes img").forEach((image) => image.addEventListener("load", reportHeight, { once: true }));
  reportHeight();
}
